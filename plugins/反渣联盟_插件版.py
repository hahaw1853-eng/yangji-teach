...
#[description: 反渣联盟自动签到插件<br>指令:反渣联盟、反渣联盟执行、反渣联盟查询、反渣运行<br>格式：账号-----密码<br>内置定时签到]
#[param: {"required":false,"key":"dd_FZ_PluginsData.proxy","bool":false,"placeholder":"可选,代理地址","name":"代理","desc":"代理地址,支持 ip:端口、http://ip:端口、socks5://ip:端口、账号:密码@ip:端口"}]
...
def parse_proxy_api(text: str) -> dict:
    """解析代理API返回，支持 ip:port username password 格式"""
    text = text.strip()
    if not text:
        return None
    # 尝试空格分隔：ip:port username password
...
def normalize_proxy(proxy: str) -> str:
    ...
    if not proxy:
        return ""
    # 如果是代理API地址（http开头），先请求获取代理
    if proxy.startswith("http"):
        try:
            import urllib.request
            req = urllib.request.Request(proxy, headers={"User-Agent": "Mozilla/5.0"}, method="GET")
