...
class ProxyManager:
    ...
    def _extract_proxies(self, response_text: str) -> List[str]:
        ...
        # 处理 host:port 用户名 密码 格式（空格分隔，每行一个代理）
...
def recognize_captcha(image_base64: str) -> str:
    """验证码识别"""
    try:
        ocr_url = 'http://172.17.0.1:8001/ocr'
...
=================="""
            ...
            if wait_choice.lower() == 'y':
                ...
                sender.reply(
                    ...
                    f"🚀 用户错峰: {user_jitter_ms}ms\n"
                    f"🎯 策略: 6轮错峰（[-100 0 50 150 300 500]ms）+ 预热独立代理\n"
                    f"⏳ 等待: {int(wait_seconds)}秒\n"
                    f"======================"
                )

                # === 抢兑策略：3轮错峰 + 每轮独立代理 + 每轮并发重试 ===
                ROUND_COUNT = 6
                ROUND_OFFSETS_MS = [-100, 0, 50, 150, 300, 500]  # 触发点（毫秒，负数=整点前提前卡位）
                ROUND_ATTEMPTS = 4                  # 每轮并发请求次数
                WARMUP_LEAD = 10                    # 整点前预热（秒）
                REFRESH_LEAD = 5                    # 到点前刷新登录会话（秒）
                ...
                # 4. 预热阶段：整点前 WARMUP_LEAD 秒为每轮预取独立代理（每次请求代理API只取一个）+ 预热连接
                ...
                # 5. 到点前 REFRESH_LEAD 秒刷新登录会话（防止会话过期）
                ...
                def single_withdraw(acc_info, proxy, session):
                    ...
                    try:
                        if session:
                            ...
                            resp = requests.get(
                                withdraw_url, params=params,
                                headers={"User-Agent": generate_kuwo_ua(acc_info.get('phone_raw', ''))},
                                verify=False, timeout=8
                            )
                        result = resp.json()
                ...
                def run_round(acc_info, round_idx):
                    """某一轮抢兑：等待触发点 → 用预热好的代理并发 ROUND_ATTEMPTS 次"""
