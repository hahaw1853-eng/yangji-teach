...
def format_result(result: dict) -> str:
    ...
    lines.append(f"  💰 +{earned} (总{total})")

    ad_err = result.get("ad_error", "")
    if ad_err:
        if "上限" in ad_err or "已用完" in ad_err:
            lines.append("  ⚠️ 广告已达上限")
        elif "未启用" in ad_err:
            lines.append("  ⚠️ 广告未启用")
        else:
            lines.append(f"  ⚠️ {ad_err[:12]}")

    checkin = result.get("checkin", "")
    if checkin:
        lines.append(f"  📝 {checkin}")
