...
#[description: 无忧计划自动任务插件，内置每日签到与看广告赚金币！支持立即执行与定时执行。<br>指令:无忧计划、无忧计划执行、无忧计划任务检测、无忧运行<br>格式：手机号#密码#UA(可选)<br>内置定时检测与自动执行任务]
...
def get_owner_nickname(owner_id: str) -> str:
    """获取用户昵称（用于艾特）"""
    try:
        # 尝试从框架获取昵称，不同框架接口可能不同
        s = middleware.Sender(owner_id)
        return s.getUserName() or owner_id
    except Exception:
        return owner_id
...
class WuYouPlan:
    ...
    def run(self) -> dict:
        ...
        return {
            "account": self.account,
            "success": True,
            "nickname": nickname if 'nickname' in dir() else "",
...
def format_result(result: dict) -> str:
    ...
    lines.append(f"💰 金币: +{earned} (当前 {total})")
    ...
    if task_list:
        ...
        lines.append(f"📋 任务: {completed}/{len(task_list)} 完成 | {claimed} 已领 | {pending} 待领")

    # 自动领取的任务
    claimed_tasks = result.get("claimed_tasks", [])
    if claimed_tasks:
        lines.append(f"🎁 自动领取: {', '.join(claimed_tasks[:3])}")
        if len(claimed_tasks) > 3:
            lines.append(f"   等共 {len(claimed_tasks)} 个任务奖励")
...
def format_summary(results: list) -> str:
    ...
    lines = []
    lines.append("╔════════════════════════════════════╗")
    lines.append("║      📊 无忧计划执行汇总           ║")
    lines.append("╠════════════════════════════════════╣")
    lines.append(f"║ 账号总数: {total_accounts:>3} 个               ║")
    lines.append(f"║ 成功: {success_count:>3} | 失败: {fail_count:>3}           ║")
    lines.append(f"║ 总获得金币: +{total_earned:>5}               ║")
    lines.append("╚════════════════════════════════════╝")
...
def Administration():
    ...
    if choice == 1:
        ...
        for i, acc in enumerate(accounts, 1):
            ...
            msg += f"║ [{i}] {mask_account(acc):>20}{owner_tag:>10} ║\n"
        msg += f"╠════════════════════════════════════╣\n"
