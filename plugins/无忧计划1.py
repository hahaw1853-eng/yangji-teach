...
class WuYouPlan:
    def __init__(self, account, password, device_id="", ua="", proxy_api=""):
        ...
        self.result_lines = []
        self.ad_error = ""  # 广告错误信息
        self.checkin_status = ""  # 签到状态
    ...
    def checkin(self):
        ...
        try:
            ...
            if data2.get("ok"):
                ...
                self.checkin_status = f"签到+{claim_coins}"
            else:
                err = str(data2)[:100]
                self.log(f"  领取签到奖励失败: {err}")
                if "已领取" in err:
                    self.checkin_status = "签到已领"
                else:
                    self.checkin_status = "签到失败"
        except Exception as e:
            self.log(f"  领取签到奖励异常: {e}")
            self.checkin_status = "签到异常"
        return data
    ...
    def watch_ads(self, account_max_views=None):
        ...
        if not self.ad_error:
            if success_count == 0 and fail_count > 0:
                self.ad_error = "广告全部失败"
            else:
                self.ad_error = f"广告{success_count}/{success_count+fail_count}"
...
def format_result(result: dict) -> str:
    ...
    if not result["success"]:
        return f"❌ {acc} | 失败:{result.get('error', '未知')}"[:60]
    ...
    name_part = acc
    if nickname:
        name_part += f"({nickname})"
    parts.append(name_part)

    if earned > 0:
        parts.append(f"+{earned}💰")
    else:
        parts.append("+0💰")
    ...
    if task_list:
        ...
        parts.append(f"任务:{completed}/{len(task_list)}")

    return " | ".join(parts)[:80]  # 限制长度防止换行
...
def format_summary(results: list) -> str:
    ...
    lines = []
    lines.append(f"📊 汇总: {total_accounts}个账号 | 成功{success_count} | 失败{fail_count} | 总收益+{total_earned}💰")

    # 失败详情
    fails = [r for r in results if not r["success"]]
    if fails:
        for r in fails:
            lines.append(f"❌ {mask_account(r['account'])}: {r.get('error', '未知')}"[:50])
...
def bind():
    ...
    if account in accounts:
        ...
        sender.reply(f"✅ {mask_account(account)} 信息已更新！")
    else:
...
def Administration():
    ...
    if choice == 1:
        ...
        sender.reply(
            f"⚠️ 确认删除 {mask_account(selected)}?\n"
            "[y]确认 [n]取消"
        )
